...
use ModelParams
      ...
      if (CP%model.eq.theta_bin) then        
         ...
         if ((z.ge.binned_z(1)).and.(z.le.binned_z(nsteps))) then
            ...
            wde = ispline(binned_z(nsteps), binned_z, binned_w, b1, c1, d1, nsteps)
         end if
      end if
      ...
      do j=1, nsteps
         ...
         do i=1,numint
            redint(i)=0._dl+(i-1)*binned_z(j)/(numint-1)
         end do
...
!         ipid = getpid()
         ...
         write(wbins, "(10f15.7)"     ) (CP%wb(i)+1,i=1,CP%nb) !python parser struggles with scientific notation negatives: using floats here
!         write(lencorr, "(10E15.7)"  ) CP%corrlen
         write(lbins, "(10E15.7)"     ) (CP%lb(i),i=1,nlbins)
